      ...
      {buttons.map((btn) => {
        ...
        return (
          <button
            ...
            className={`${bgColor} transition-all text-4xl font-extrabold h-20 md:h-24 rounded-2xl flex items-center justify-center select-none ${disabled ? 'opacity-50 cursor-not-allowed pointer-events-none border-b-0 translate-y-1' : 'cursor-pointer'}`}
          >
            {btn}
          </button>
        );

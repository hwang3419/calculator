...
export const BearRescueScene: React.FC<BearRescueSceneProps> = ({ stage, reaction, stageProgress, answersPerStage }) => {
  ...
  return (
    <div className="mb-3 w-full overflow-hidden rounded-[2rem] border-4 border-white bg-gradient-to-br from-emerald-100 via-sky-100 to-lime-100 p-4 shadow-sm md:mb-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.22em] text-green-700">Rescue The Bear</div>
          <div className="mt-1 text-sm font-extrabold text-green-900 md:text-base">{STAGE_COPY[safeStage]}</div>
          {safeStage < 3 && (
      ...
      <div className="mb-4 h-3 overflow-hidden rounded-full bg-white/70">
        <div
      ...
      <div className="relative overflow-hidden rounded-[2rem] border-4 border-white/60 bg-white/40">
        <div
        ...
        {reaction === 'sad' && (
          <div className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-2 text-xs font-black text-rose-500 shadow-sm">
            Oops... try again
          </div>
        )}
